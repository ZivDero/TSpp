...
Point2D Conquer_Clip_Text_Print(char const*, Surface& surface, Rect const& rect, Point2D const& pt, ColorScheme* fore, int back = 0, TextPrintType flag = TPF_8POINT | TPF_DROPSHADOW, int width = -1, int const* tabs = nullptr);

Point2D Simple_Text_Print(const char* text, Surface& surface, Rect& rect, Point2D& xy, ConvertClass* drawer, unsigned fore, unsigned back = COLOR_TBLACK, TextPrintType flag = TPF_8POINT | TPF_DROPSHADOW, int a8 = 1);
